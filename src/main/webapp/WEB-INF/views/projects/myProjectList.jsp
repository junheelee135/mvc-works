	...
	<main id="main-content">
		...
		<div class="project-container">
							...
							<tr>
								...
								<td><c:choose>
										<c:when test="${p.status == '1'}">
											<span class="status-badge badge-ready"><span
												class="status-dot"></span>시작전</span>
										</c:when>
										<c:when test="${p.status == '2'}">
											<span class="status-badge badge-inprogress"><span
										...
										<c:when test="${p.status == '4'}">
											<span class="status-badge badge-finished"><span
												class="status-dot"></span>종료</span>
										</c:when>
										<c:when test="${p.status == '5'}">
											<span class="status-badge badge-delayed"><span
												class="status-dot"></span>지연</span>
										</c:when>
										<c:when test="${p.status == '6'}">
											<span class="status-badge badge-stop"><span
			...
			<div class="d-flex justify-content-center py-4 border-top">
				${dataCount == 0 ? "등록된 게시글이 없습니다" : paging}</div>
		</div>
	</main>
