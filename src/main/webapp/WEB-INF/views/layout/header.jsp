...
<nav class="navbar-custom">
	...
	<div class="navbar-right">
		<span>${sessionScope.member.name}
			${sessionScope.member.gradeName}님 환영합니다.</span>

		<%-- 알림 버튼 + 토글 드롭다운 --%>
		<div class="noti-wrap" id="notiWrap">
			<button class="nav-icon-btn position-relative" id="notiBtn"
				type="button">
			...
			<div class="noti-panel" id="notiPanel">
				<div class="noti-panel-header">
					<span class="noti-panel-title"><i class="far fa-bell me-1"></i>
						알림</span>
					<button class="noti-read-all-btn" id="notiReadAllBtn" type="button">모두
						읽음</button>
				</div>
		...
		<sec:authorize access="isAuthenticated()">
			...
			<a href="${pageContext.request.contextPath}/member/logout"
				class="nav-icon-btn" title="로그아웃"> <i
			...
			<div class="dropdown">
				<c:choose>
					<c:when test="${not empty avatar}">
						<img
							src="${pageContext.request.contextPath}/uploads/member/${avatar}"
				...
				<ul class="dropdown-menu dropdown-menu-end">
					<li><a href="${pageContext.request.contextPath}/member/pwd"
						class="dropdown-item"><i class="fas fa-user-edit me-2"></i>정보수정</a></li>
				</ul>
			</div>
		</sec:authorize>
	</div>
